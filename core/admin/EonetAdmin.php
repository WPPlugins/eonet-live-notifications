...
class EonetAdmin extends Eonet
{
    ...
    public function ajaxSaveSettings() {
        ...
        if($value || $eonet_settings == get_option($option_field)) {
            ...
            $response['title'] = __('Done, everything is safe !', 'eonet-live-notifications');
            $response['content'] = __('Your settings have been saved successfully.', 'eonet-live-notifications');
        } else {
            $response['status'] = 'error';
            $response['title'] = __('Something went wrong...', 'eonet-live-notifications');
            $response['content'] = __('We\'ve not been able to save your settings, please try again later.', 'eonet-live-notifications');
        }
    ...
    public function ajaxResetSettings() {
        ...
        if($value) {
            $response['status'] = 'success';
            $response['title'] = __('Done, settings have been reset !', 'eonet-live-notifications');
    ...
    public function adminMenus(){
        // main page :
        add_menu_page(
            __( 'Eonet Dashboard', 'eonet-live-notifications' ),
            __( 'Eonet','eonet-live-notifications' ),
            'manage_options',
            'eonet',
    ...
    static public function getThemes() {
        $themes = array(
            array(
                'name' => __('Woffice Intranet/Extranet theme', 'eonet-live-notifications'),
                ...
                'premium' => true,
                'features' => array(
                    __('Projects Management','eonet-live-notifications'),
                    __('Complete Wiki/Knowledge base','eonet-live-notifications'),
                    __('Members map & directory','eonet-live-notifications'),
                    __('Responsive layout (mobile & tablet)','eonet-live-notifications'),
                    __('Cover picture','eonet-live-notifications'),
                    __('Private / Public pages','eonet-live-notifications'),
                    __('Files Manager','eonet-live-notifications'),
                    __('Events calendar','eonet-live-notifications'),
                )
            )
        );
