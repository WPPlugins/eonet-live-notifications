...
class EonetComponents{
    ...
    public function addSetting($links) {
        ...
        $link = '<a href="' . admin_url( 'plugins.php?page=eonet&eo_settings='.$this->current_component ) . '">'. __('Settings','eonet-live-notifications') .'</a>';
    ...
    public function ajaxStateChange() {
        ...
        if(!empty($method) && !empty($component)) {
            ...
            if($method == "deactivate") {
                ...
                $response['title'] = __('Extension has been deleted !', 'eonet-live-notifications');
                $response['content'] = __('The page will be refreshed in a few seconds...', 'eonet-live-notifications');
            }
            if($method == "activate") {
                $installed = $this->activate($component);
                if($installed == false){
                    $response['status'] = 'error';
                    $response['title'] = __('We couldn\'t install it...', 'eonet-live-notifications');
                    $response['content'] = __('Please try from your plugin page.', 'eonet-live-notifications');
                }
                /*$response['status'] = 'success';
                $response['title'] = __('We\'re installing the extension !', 'eonet-live-notifications');
                $response['content'] = __('The page will be refreshed in a few seconds...', 'eonet-live-notifications');*/
            }
        } else {
            $response['status'] = 'error';
            $response['title'] = __('Something went wrong...', 'eonet-live-notifications');
    ...
    static public function getComponents($slug = '') {
        ...
        $components['live-search'] = array(
            'name' => __('Live Search', 'eonet-live-notifications'),
            'version' => '1.0.0',
            'icon' => 'ion-ios-search',
            'premium' => false,
            'description' => __('Enhance your Wordpress search bar with customizable live search through all your post types and members.', 'eonet-live-notifications'),
        );

        $components['frontend-publisher'] = array(
            'name' => __('Frontend Publisher', 'eonet-live-notifications'),
            'version' => '1.0.0',
            'icon' => 'ion-ios-compose-outline',
            'premium' => false,
            'description' => __('Allow yourself, your team or your user to edit and publish any Wordpress post or page.', 'eonet-live-notifications'),
        );

	    $components['manual-user-approve'] = array(
		    'name' => __('Manual User Approve', 'eonet-live-notifications'),
		    'version' => '1.0.0',
		    'icon' => 'ion-person-add',
		    'premium' => false,
		    'description' => __('Set a pending status to user regsitered on your site, you can manually approve or deny them.', 'eonet-live-notifications'),
	    );

        $components['live-notifications'] = array(
            'name' => __('Live Notifications', 'eonet-live-notifications'),
            'version' => '1.0.0',
            'icon' => 'ion-ios-loop',
            'premium' => false,
            'description' => __('Enables live notifications for all your users to get better interactions within your site.', 'eonet-live-notifications'),
        );

        $components['project-manager'] = array(
            'name' => __('Project Manager', 'eonet-live-notifications'),
            'version' => '0.0.0',
            'icon' => 'ion-briefcase',
            'premium' => false,
            'description' => __('Enables a complete project management tool for your site.', 'eonet-live-notifications'),
        );

        $components['social-connect'] = array(
            'name' => __('Social Connect', 'eonet-live-notifications'),
            'version' => '0.0.0',
            'icon' => 'ion-ios-cloud-download-outline',
            'premium' => false,
            'description' => __('Allow your visitor to login or register using Facebook, Twitter, Slack or Linkedin.', 'eonet-live-notifications'),
        );
